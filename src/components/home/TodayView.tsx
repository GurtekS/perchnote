...
export function TodayView() {
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      toast.error(`Could not create a meeting: ${String(e)}`);
    } finally {
      setIsCreating(false);
    }
  };
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-xl mx-auto px-6 py-10 space-y-8 animate-fade-in">
        ...
        <div className="space-y-0.5">
          <p className="text-[13px]" style={{ color: "var(--meeting-meta-color)" }}>{getGreeting()}</p>
          <p className="text-[26px] font-semibold tracking-[-0.025em] text-text-primary leading-tight">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-[15px] font-semibold text-white transition-all hover:brightness-110 active:scale-[0.99] disabled:cursor-wait disabled:opacity-60"
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            {thisWeekCount > 0 && (
              <span className="text-[11px]" style={{ color: "var(--meeting-meta-color)" }}>
          ...
          {todayMeetings.length === 0 ? (
            ...
            >
              ...
              <p className="text-[11px] mt-1" style={{ color: "var(--meeting-meta-color)" }}>
                {isEmptyAccount
              ...
              {isEmptyAccount && (
                <div className="mt-4 flex flex-col items-stretch justify-center gap-2 sm:flex-row">
                  <button
                    ...
                    disabled={isCreating}
                    aria-busy={isCreating}
                    className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-accent px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-accent/90 disabled:cursor-wait disabled:opacity-60"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-border px-3 py-2 text-sm font-medium text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary"
                  >
                    <ListChecks size={14} />
                    Open setup guide
                  </button>
              ...
              {todayMeetings.map((m) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-left transition-all hover:brightness-110"
                    style={{
                      background: upcoming
                      ...
                      border: upcoming
                        ...
                        : "1px solid var(--glass-header-border)",
                    }}
                  >
                    {/* Time */}
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-[13px] font-medium text-text-primary truncate block leading-tight">
                        {m.title}
                      </span>
                      {duration && (
                        <span className="text-[11px]" style={{ color: "var(--meeting-meta-color)" }}>
          ...
          <div>
            ...
            <div className="space-y-0.5">
              {recentMeetings.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    onClick={() => navigate({ to: "/meeting/$id", params: { id: m.id } })}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left hover:bg-bg-hover transition-colors"
                  >
                    <span className="flex-1 text-[13px] text-text-primary truncate">{m.title}</span>
                    {duration && (
                      <span className="text-[11px] shrink-0" style={{ color: "var(--meeting-meta-color)" }}>
                        {duration}
                      </span>
                    )}
                    <span className="text-[11px] shrink-0" style={{ color: "var(--meeting-meta-color)" }}>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1 px-3 py-2 text-[12px] rounded-lg transition-colors hover:bg-bg-hover"
