...
- In the table view, drag the right edge of any header — Title and every
  column — to resize it. Your widths are remembered. Arrow keys resize a
  focused grip, and double-clicking it resets that column.

### Empty recordings don't stick around
- Stopping a recording that captured no audio — a mis-click on record, or
  a capture that never started — no longer leaves an empty meeting in your
  list. If there's no audio and nothing was typed or transcribed, the
...
- A meeting left on its placeholder title ("Untitled Meeting",
  ...
  the toast. Strictly scoped: titles you typed are never touched (even
...
- **Trashing, archiving, or renaming a calendar-synced meeting now
  sticks** — sync resurrected/reverted them every 5 minutes.
...
- If you've saved a voice profile, recurring voices are now recognized
  and named automatically after each recording — strictly on-device,
...
- **Accuracy pass (on by default, toggleable)** — when a recording
  ...
  you edited a segment in the meantime, your edits win — the pass
...
- A recipe can now carry a **scope** (`folder:ClientX after:2026-03`) and
  run across your recent meetings instead of just the open one — same
  retrieval as Ask AI's All-meetings mode.
...
- AI notes now render one step smaller and tighter than the typing
  surface — generated output stopped looking longer than it is.
...
- An AI-generated task could be assigned to someone who was never in
  ...
  speaker names, or the attendee list — otherwise the task is kept
  but unowned.
...
- Two meetings with the same title on the same day no longer share one
  vault file (they used to alternate "conflict" warnings forever) —
  the second meeting now mirrors to "Title (2).md", and existing
...
- **Deep-link verbs** — `perchnote://record/start?title=Standup`,
  ...
  the two automation schemes they exist for — a webpage can't launder
...
- **"Your June"** — a few honest paragraphs about your month,
  ...
  that was sent — the privacy claim is inspectable, not asserted.
